# Python script to plot the error in the exponential approximation
...
# vectors
DeltaXp = np.linspace(0.1,2,30)
DeltaX_norm = np.linspace(0,1,30) # normalized version of DeltaX
...
for lin in range(len(DeltaXp)):
    ...
    for col in range(len(DeltaX_norm)):
        ...
        # trasn
        Erro[lin,col] = 1 - Error_linear_part*Error_exp_part 
...
plt.savefig('./graphs/Errors3D.pdf', bbox_inches='tight')
plt.savefig('./graphs/Errors3D.svg', bbox_inches='tight')
