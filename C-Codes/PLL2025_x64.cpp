...
// considerar a simulação:
// C:\Users\cleit\Documents\GitHub\PLL-2024\Simulations\simulacoes_20251017\PLL_alfa_beta_20251017_c


void PLL2025_x64(uint1_t sinc,  data_t v_alpha_in,
                                data_t v_beta_in,
                                data_t *wout,
                                ){
    ...
    static data_t v1_d_limpo; // Representa a amplitude
    static data_t v1_q_limpo; // Representa o erro de fase
    ...
    static data_t v2_d_comp;
    static data_t v2_q_comp;

    static data_t v1_alpha;
    static data_t v1_beta;

    static data_t delta2 = 0;
    static data_t V2_amp = 0;
    ...
    if(aux_sinc == sinc){
        ...
        // Aquisição das variáveis de entrada
        v_alpha = v_alpha_in;
        v_beta = v_beta_in;


        // Computação: cálculo das componentes de compensação
        v1_d_comp = V2_amp*cos_2000(2*theta1 + delta2);
        v1_q_comp = V2_amp*sin_2000(2*theta1 + delta2);

        v2_d_comp = v1_d_limpo*cos_2000(2*theta1);
        v2_q_comp = v1_d_limpo*sin_2000(2*theta1);

        
        // Computação: Variáveis algébricas de sequência positiva
        v1_d = sin_2000(theta1)*valpha - cos_2000(theta1)*vbeta;
        v1_q = cos_2000(theta1)*valpha + sin_2000(theta1)*vbeta;

        v1_d_limpo = v1_d + v1_d_comp;
        v1_q_limpo = v1_q - v1_q_comp;

        // Computação: Variáveis algébricas de sequência negativa
        v2_d = sin_2000(theta1)*valpha + cos_2000(theta1)*vbeta;
        v2_q = cos_2000(theta1)*valpha - sin_2000(theta1)*vbeta;

        v2_d_limpo = v2_d - v2_d_comp;
        v2_q_limpo = v2_q + v2_q_comp;
        ...
        wi_new = wi + Ts*Ki*v1_q_limpo;

        w = wi_new + Kp*v1_q_limpo + w0;
        ...
        if(theta>=pi2){  
            ...
            theta1_new = theta1 + Ts*w;
        }



        // Computação: Variáveis algébricas de avaliação
        v1_alpha = v1_d_limpo*sin_2000(theta1);
        v1_beta = -v1_d_limpo*cos_2000(theta1);
